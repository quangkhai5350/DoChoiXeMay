    ...
    public class SerialController : Controller
    {
        ...
        public ActionResult AddNewSerial(Ser_sp SerSP,int SoSerialN)
        {
            ...
            try
            {
                ...
                {
                    string SN = Utils.XString.MakeAotuSN()+  Utils.XString.GetRanDomOTP(7) + SerSP.LoSanXuat;
                    var kt = dbc.Ser_sp.Where(kh => kh.SerialSP.Contains(SN)).ToList();
                    if (kt.Count() > 0)
                    {
                        //Nếu S/N bị trùng, random 20 lần
                        for (int j = 0; j < 20; j++) {
                            SN= Utils.XString.MakeAotuSN() + Utils.XString.GetRanDomOTP(7) + SerSP.LoSanXuat;
        ...
        public ActionResult DeleteSerialBox()
        {
            try
            {
                ...
                var XoaChitietXN = dbc.Database.ExecuteSqlCommand(sql);
                Session["ThongBaoSerialSPchuaIn"] = "Xóa tất cả S/N Box thành công.";
                return RedirectToAction("ListSerialChuaIn");
            }
            catch (Exception ex)
            {
                string loi = ex.ToString();
                Session["ThongBaoSerialSPchuaIn"] = "Có Lỗi hệ thống khi xóa S/N Box !!!";
                return RedirectToAction("ListSerialChuaIn");
            }
        }
